...
#density.shape (256, 256, 256)
    ...
    gas_energy = f['Energy']
    if(i==0):
      gas_energy_image = np.zeros((f.attrs['dims'][1],f.attrs['dims'][0]))
      print(f'Filename {fname}')
      ...
      print(list(f.keys()))
      print(f'gas_energy.shape {gas_energy.shape}')
    ...
    if((oz>=args.izmin)&(oz+nzl<=args.izmax)):
      ...
      gas_energy_image[ox:ox+nxl,oy:oy+nyl] += np.sum(gas_energy[:,:,izmin:izmax],axis=2)
    

  #save the gas_energy field
  plt.imsave(args.output,np.log10(gas_energy_image),origin='lower',cmap=f'{args.cmap}')
